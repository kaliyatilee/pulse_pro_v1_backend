...
class MerchantController extends Controller
{
    ...
    public function index()
    {
        ...
        $tstock = Reward::where('userId', $userdata->id)->where('status','approved')->sum('totalQty');

        $data['tstock'] = $tstock;

        $coinsreddmed = Transaction::where('status', 'paid')->where('user_id', $userdata->id)->count();

        $data['coinsreddmed'] = $coinsreddmed;

        $totalrevenue = Subscriptions::All()->where('user_id', $userdata->id)->sum(function($t){ 
            return $t->numlicence * $t->amount; 
        });

        $data['totalrevenue'] = $totalrevenue;

        $trendingproduct = Transaction::query()->join('rewards', 'rewards.id', '=' , 'transactions.reward_id')->where('transactions.status', 'paid')->where('transactions.user_id', Auth::user()->id)->groupBy('transactions.reward_id')->havingRaw('COUNT(*) > 1')->get()->count();
        //\DB::enableQueryLog(); // Enable query log
        //dd(\DB::getQueryLog());
        //print_r($trendingproduct);exit;
        //$trendingproduct = Transaction::where('status', 'paid')->where('user_id', $userdata->id)->havingRaw('COUNT(*) > 5')->count();

        $data['trendingproduct'] = $trendingproduct;
        ...
        return view('merchant.dashboard', compact('newMembersList','products'), array(
                "userdata" => $userdata,
    ...
    public function productlistfilter(Request $request)
    {
        ...
        $products = Reward::where('status', trim($_GET['filter']))->get();
