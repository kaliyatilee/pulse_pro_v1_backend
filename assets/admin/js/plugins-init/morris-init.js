(function ($) {
    ...
    var dzMorris = function () {
        ...
        const modal = (members, bmi) => {
            let rows = [];

            for(let i= 0; i < members.length; i++) {
                let date = new Date(members[i].created_at)
            ...
            let htmlx = `<table id="wellnessChart" class="w-100 max-h-100 p-20">
                            <thead>
                                <tr>
                                    <th>Pulse ID</th>
                                    <th>BMI</th>
            ...
            $(document).ready(function() {
                $('#wellnessChart').DataTable( {
                    ...
                    buttons: [
                        {
                            extend: 'excel',
                            title: `Wellness Report ${date.toLocaleDateString("en-US")}`,
        ...
        var donutChart = async function () {
            ...
            let donut = Morris.Donut({
                ...
                data: [
                    {
                        label: "\xa0 \xa0 Underweight \xa0 \xa0",
                        value: data.bmi.underweight,

                    },
                    {
                        label: "\xa0 \xa0 Overweight \xa0 \xa0",
                        value: data.bmi.overweight,
                    },
                    {
                        label: "\xa0 \xa0 Obese \xa0 \xa0",
                        value: data.bmi.obese
                    }, {
                        label: "\xa0 \xa0 Normal \xa0 \xa0",
                        value: data.bmi.normal
                    }],
                resize: true,
                redraw: true,
                colors: ['#a742b8', '#f0990e', '#f00000', '#1bd084'],
                //responsive:true,

            }).on('click', function (i, row) {
                let bmi = row.label.toLowerCase().trim();
                modal(data.members[bmi], bmi);
            });

            for(let i = 0; i < donut.segments.length; i++) {
                donut.segments[i].handlers['hover'].push( function(i){
                    const wellnessChats = document.querySelectorAll('.wellnessComparison');

                    wellnessChats.forEach((item, index) => {
                        if (item.id.toString().trim().includes(donut.data[i].label.toString().trim())) {
                            wellnessChats[index].classList.remove('lineChart-hidden');
                            wellnessChats[index].classList.add('lineChart-active');
                        }
                        else {
                            wellnessChats[index].classList.add('lineChart-hidden');
                        }
                    })
                });
            }
